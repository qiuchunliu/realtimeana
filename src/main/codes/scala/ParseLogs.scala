...
object ParseLogs {
  ...
  def splitLog(rdd: RDD[ConsumerRecord[String, String]]): RDD[JSONObject] = {
    val rddjson: RDD[JSONObject] = rdd.map(e => {
      ...
      if (str.contains("reChargeNotifyReq")) {  // 校验订单的类型
        js = JSON.parseObject(str)
      }
      js
    })
    rddjson.filter(_ != null)  // 过滤调null值
  }
  ...
  def save_failure_orders_per_city_per_day(v: RDD[((String, String), Int)]): Unit = {
    ...
    def res(v: RDD[((String, String), List[Double])]): Unit = {
      v.foreachPartition(p => {
        ...
        p.foreach(e => {
          sql = "insert into res4(order_date,province,isCharged,chargeFee) " +
            "values('" + e._1._1 + "','" + e._1._2 + "'," + e._2.head + "," + e._2(1) + ");"
